  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Review campaign images and videos from one place."
      />
      ...
      <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
        <SectionCard
          title="Products"
          description="Select a product to review its marketing assets."
        >
        ...
        {isLoading ? (
          <SectionCard
            title="Loading Marketing Media"
            description="Loading marketing media for the selected product."
          >
          ...
          <SectionCard
            ...
            description="Review thumbnails, duration, and display order for each asset."
          >
            <div className="flex flex-wrap items-center gap-3">
              <StatusBadge label={`${selectedRow.totalAssets} total`} variant="info" />
              <StatusBadge label={`${selectedRow.photoCount} photos`} variant="neutral" />
            ...
            {selectedRow.heroAsset ? (
              <div className="rounded-2xl border border-zinc-200 bg-zinc-50 px-5 py-4">
                <p className="text-xs uppercase tracking-[0.14em] text-zinc-400">
                  Hero Asset
                </p>
                <p className="mt-2 text-sm font-medium text-zinc-900">
                  {selectedRow.heroAsset.url}
                </p>
                <p className="mt-1 text-sm text-zinc-500">
                  {selectedRow.heroAsset.type} • sort order{" "}
                  {selectedRow.heroAsset.sortOrder}
                </p>
              </div>
            ) : null}
            <div className="overflow-hidden rounded-2xl border border-zinc-200">
              <table className="min-w-full divide-y divide-zinc-200">
                <thead className="bg-zinc-50">
                  <tr>
                    {["Asset URL", "Type", "Duration", "Display Order"].map((header) => (
                      <th
                        key={header}
                  ...
                  {selectedRow.items.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td className="px-4 py-4 text-sm text-zinc-700">
                        {item.duration ? `${formatCompactNumber(item.duration)} sec` : "N/A"}
                      </td>
          ...
          <SectionCard
            ...
            description="Choose a product to review its marketing assets."
          >
            <div className="rounded-2xl border border-dashed border-zinc-200 bg-zinc-50 px-5 py-10 text-sm text-zinc-500">
