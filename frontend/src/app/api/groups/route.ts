...
export async function GET(req: NextRequest) {
  return bffGet({ path: '/api/product-groups', request: req, requiredRoles: ['Admin', 'Staff'] })
}

export async function POST(req: NextRequest) {
  const v = await validateBody(req, CreateGroupSchema)
  if (v.error !== null) return v.error
  return bffMutate({ path: '/api/product-groups', method: 'POST', request: req, body: v.data, requiredRoles: ['Admin'] })
}
