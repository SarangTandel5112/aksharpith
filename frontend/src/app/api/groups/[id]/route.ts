...
export async function GET(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  return bffGet({ path: `/api/groups/${id}`, request: req, requiredRoles: ['Admin', 'Staff'] })
}
...
export async function PATCH(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  return bffMutate({ path: `/api/groups/${id}`, method: 'PATCH', request: req, body: v.data, requiredRoles: ['Admin'] })
}

export async function DELETE(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  const { id } = await props.params
  return bffMutate({ path: `/api/groups/${id}`, method: 'DELETE', request: req, requiredRoles: ['Admin'] })
}
